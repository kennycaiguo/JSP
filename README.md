##### 1.组件介绍厚溥upload(上传组件)是基于js适用于struts2，springmvc等当下流行的各种前端框架上传文件的web组件；
##### 2.组件工作原理通过js将文件上传到后台，并接受后台返回json数据（文件储存地址）。
##### 3.组件演示（单图）详细演示请点击如下[链接](http://103.73.49.112:8080/struts2/)查看。
##### 4.组件使用
######1）单文件组件只需要导入hpUpload.js，layer.js，jquery，hpUpload.css即可使用，当然你要在后台进行一些接收文件的处理,这里以struts2为例（默认使用id为“kdUploadSelect”，也可以自定义id）
前端：
```
...
自定义id进行上传</br>
<img id="anyLable"  style="width: 100px;height: 100px" src="img/Chrysanthemum.jpg">
</body>
 <script type="text/javascript">
  $(function(){
    ...
    //  自定义id绑定
 	  $.hpUpload("uploadFile.hp",{"path":"/kd/img"},function(json){
		  console.log(json);
		  $("#anyLable").attr('src',"./"+json.imgPath);
	  },false,"anyLable"); 
  })
  </script>
</html>
...
##### 5.组件演示（多图）详细演示请点击如下[链接](http://103.73.49.112:8080/struts2/imgList.jsp)查看

######2）多文件上传组件只是在单文件组件的基础上改下名字 (单文件是$.hpUpload ,  多文件是$.hpUpload_plus.viewUpload)
如下为多文件上传前端(后端不用变动)：
```
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
